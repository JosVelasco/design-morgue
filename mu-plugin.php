...
// Allow Gutenberg only on the workshop checklist page.
add_filter( 'use_block_editor_for_post', function ( $use, $post ) {
	return 'workshop-checklist' === $post->post_name ? true : false;
}, 10, 2 );
...
add_action( 'admin_head', function () {
	...
	echo '<style>
		#design-morgue-notice { background:#0d0d0d; border-left:4px solid #c0392b; color:#e8e0d0; padding:18px 24px; margin:16px 20px 0; }
		...
		#design-morgue-notice .morgue-tag { font-size:11px; letter-spacing:2px; text-transform:uppercase; color:#555; margin-bottom:6px; display:block; }
	</style>';
} );

// Inject fonts into the block editor canvas too.
